...
async def cmd_score_report() -> None:
    """
    Distributia scorurilor lead-urilor recente. Util ca sa vezi
    daca pragul de scoring (7) e prea strict — daca majoritatea
    lead-urilor pica sub 7, asta explica de ce "gasesti putine".
    """
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Job).where(Job.is_b2b.is_(False))
    ...
    buckets = {"0-3": 0, "4-6": 0, "7-9": 0, "10-12": 0, "13-15": 0}
    for j in jobs:
        s = j.lead_score or 0
        if s <= 3: buckets["0-3"] += 1
        elif s <= 6: buckets["4-6"] += 1
        elif s <= 9: buckets["7-9"] += 1
        elif s <= 12: buckets["10-12"] += 1
        else: buckets["13-15"] += 1

    above_threshold = sum(v for k, v in buckets.items() if k not in ("0-3", "4-6"))

    print(f"\n  {'='*60}")
    print(f"  DISTRIBUTIE SCOR — ultimele {len(jobs)} lead-uri")
    print(f"  {'='*60}")
    for bucket, count in buckets.items():
        bar = "█" * int(40 * count / max(1, len(jobs)))
        marker = "  <- SUB PRAG (7)" if bucket in ("0-3", "4-6") else ""
        print(f"  {bucket:>6}: {count:>4}  {bar}{marker}")
    print(f"  {'-'*60}")
    print(f"  Trec de prag (>=7): {above_threshold}/{len(jobs)} "
          f"({100*above_threshold/len(jobs):.0f}%)")
    if above_threshold / len(jobs) < 0.1:
        print(f"\n  ⚠️  Sub 10% trec pragul — scoring-ul e probabil PREA STRICT")
        print(f"      pentru piata ta. Considera sa scazi SCORE_THRESHOLD_DEPLOY")
        print(f"      din lead_scoring.py de la 7 la 4-5.")
    print(f"  {'='*60}\n")
...
async def main() -> None:
    ...
    sub.add_parser("score-report", help="Distributia scorurilor — diagnoza prag prea strict")
