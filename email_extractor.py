...
async def enrich_b2b_jobs(limit: int = 100) -> None:
    """Wrapper backward-compat — enrichment doar pe B2B."""
    await enrich_emails(limit=limit, only_b2b=True)


async def enrich_emails(limit: int = 100, only_b2b: bool = False) -> None:
    """
    Itereaza prin Job-uri fara email si incearca enrichment.

    only_b2b=True  -> doar lead-uri Companies House (rata succes mare,
                       firme au site cunoscut)
    only_b2b=False -> TOATE lead-urile (OLX RO + Gumtree UK + B2B).

    ATENTIE onesta: pentru meseriasi individuali (OLX/Gumtree), rata
    de succes va fi MULT mai mica decat pe B2B — majoritatea n-au
    site propriu (de-asta iti cumpara unul de la tine). E normal sa
    vezi rata de gasire 10-20% pe astea, vs 30-50% pe Companies House.
    ...
    query = select(Job).where(
        Job.email.is_(None),
        Job.status.in_([JobStatus.SCRAPED, JobStatus.DEPLOYED]),
    )
    if only_b2b:
    ...
    if not jobs:
        logger.info("Niciun Job fara email — nimic de enriched.")
        return

    logger.info("Email enrichment: %d job-uri de procesat (only_b2b=%s)",
               len(jobs), only_b2b)
    ...
    async with httpx.AsyncClient(
        ...
        for job in jobs:
            ...
            # Extrage location din descriere (orase RO + UK)
            location = ""
            ...
            all_cities = [
                ...
                # RO
                "Bucuresti", "Cluj-Napoca", "Timisoara", "Iasi",
                "Constanta", "Craiova", "Brasov", "Galati",
                "Ploiesti", "Oradea", "Sibiu", "Arad",
            ]
            for city in all_cities:
                if city in desc:
                    location = city
                    break
            location = location or ("UK" if job.language == "English" else "Romania")

            email, website = await _find_email_for_company(
                client, job.business_name, location, breaker,
            )
